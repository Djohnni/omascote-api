...
if (tipo === "saldo_livre") {
  const valorLivre = Number(String(valor || "").replace(",", "."));
  const valoresPermitidos = [8, 18, 28, 48];

  if (!valoresPermitidos.includes(valorLivre)) {
    return res.status(400).json({ ok: false, error: "Valor de saldo inválido" });
  }

  compra = {
    titulo: "Saldo IA4Tube",
    valor_pago: valorLivre,
    credito: valorLivre,
    tipo: "saldo_livre",
    mes: ""
  };
...
c.saldo_extra = Number((Number(c.saldo_extra || 0) + credito).toFixed(2));
c.ativo = true;
